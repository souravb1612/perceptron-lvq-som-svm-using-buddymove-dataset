import numpy as np
import math as mt
data = np.genfromtxt('https://archive.ics.uci.edu/ml/machine-learning-databases/00476/buddymove_holidayiq.csv',delimiter=';')#data latih
data_uji = np.genfromtxt('https://archive.ics.uci.edu/ml/machine-learning-databases/00476/buddymove_holidayiq.csv',delimiter=';')#data uji
...
def test_data(wt,data):
    ...
    for q in range(len(data)):
        ...
        val = np.amin(res)
        if(val == xw1):
            tanaman_type = "userid1"
            winner = wt[0]
        elif (val == xw2):
            tanaman_type = "userid2"
            winner = wt[1]
        elif (val == xw3):
            tanaman_type = "userid3"
            winner = wt[2]
